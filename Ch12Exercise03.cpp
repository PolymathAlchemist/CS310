 ...
 *
 * Description:
 * This program allows the user to enter the number of local election
 * candidates, each candidate's last name, and the number of votes received
 * by each candidate. It then displays each candidate's vote total,
 * percentage of the overall vote, and the winner of the election.
 *
 * Notes:
 * The program uses dynamic parallel arrays because the assignment exercise
 * focuses on raw dynamic arrays. Other data structures would be cleaner
...
int main()
{
    ...
    const string CANDIDATE_TOTAL_PROMPT =
    ...
    do
    {
        ...
        // Input
        // ----------------------------------------------------------------
        candidate_count = get_bounded_int(
            CANDIDATE_TOTAL_PROMPT,
            MINIMUM_CANDIDATES,
            MAXIMUM_CANDIDATES
        );
 ...
 *
 * candidate_count : int
 *     User-defined number of dynamically allocated candidates to collect.
 *
 * Returns
 * -------
 * None
    ...
    for (int index = 0; index < candidate_count; index++)
    {
        ...
        while (true)
        {
            candidate_names[index] = get_candidate_last_name(
                "What is candidate " + to_string(index + 1) +
                "'s last name? "
            );
